...
class Order:
    def __init__(self,username,chargeType,chargeQuantity):
        ...
        #Generating Wait EF ET S_Fi S_Ti
...
class Bill:
    def __init__(self,order:Order,canceled = 0):
        ...
        self.real_quantity = (order.end - order.begin) * (30 if order.chargeType == 'fast' else 10) if canceled else order.chargeQuantity
...
class ChargeBoot:
    ...
    def shut(self)->list:
        ...
        if(self.busy):
            ...
            #结算正在进行的
            ans.append(Order(head.username,head.chargeType,head.chargeQuantity - (head.end - head.begin) * self.Charge_Speed))
...
class WaitArea:
    ...
    def getCarBeforenum(self,username:str):
        ...
        if(self.usr2num[username][0] == 'EF'):
            ...
            while (tail != None and tail.order.username != username):
                tail = tail.next
                ans = ans + 1
            return ans
        ...
        if (self.usr2num[username][0] == 'ET'):
            ...
            return ans
        # 在Wait_Queue中
        if (self.usr2num[username][0] == 'Wait'):
    ...
    def delord(self,username):
        ...
        elif (self.usr2num[username][0] == "ET"):
            del self.usr2num[username]
            self.emergency_slow_queue.cancel(username)
        elif(self.usr2num[username][0] == "Wait"):
    ...
    def fetch_first_fast_order(self)->Order:
        ...
        while(cur != None and cur.order.chargeType != 'fast'):
            cur = cur.next
        #print("搜索结果:",cur)
        #if(cur is not None): cur.order.show()
        ...
        return ans
    def fetch_first_slow_order(self)->Order:
    ...
    #检测是否有等待
    def haswaitF(self):
        if(self.fast_order_in_wait):return 1
        if(self.emergency_fast_queue.size):return 1
        return 0
    def haswaitS(self):
        if(self.slow_order_in_wait):return 1
        if(self.emergency_slow_queue.size):return 1
        return 0
    #用于debug
    def witness(self):
        print("EN=",self.EN)
